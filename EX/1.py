...
def schedule_courses():
    for course in courses:
        ...
        # 找到一個空位來安排課程
        for i in range(len(newSlots) - hours + 1):
            if all([newSlots[i + j] == "" for j in range(hours)]):
                # Check if the course is already scheduled on the same day
                same_day = all([newSlots[i + j] == course_name for j in range(hours)])
                if not same_day:
                    newSlots[i:i+hours] = [course_name] * hours
                    break
